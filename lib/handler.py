  ...
  with open(f"tmp/{filename}", "a", encoding="UTF-8") as r:
    ...
    for item in json['items']:
      try:
        ...
        quantity     = float(sanitizeField(item['quantity']))
        pricePerUnit = float(sanitizeField(item['pricePerUnit']))
        costAmount   = float(pricePerUnit * quantity)
        exciseRate   = float(sanitizeField(item['exciseRate']))
        excise       = float(sanitizeField(item['excise']))
        cost         = float(costAmount + excise)
        vat18        = float(sanitizeField(item['vat18']))
        vat0         = float(sanitizeField(item['vat0']))
        vatFree      = float(sanitizeField(item['vatFree']))
        exempt       = float(sanitizeField(item['exempt']))
